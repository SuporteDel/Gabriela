...
(function () {
  ...
  function wrapSection(id, title, innerHtml) {
    return `
      <section id="${id}" class="pageSection" aria-label="${title}">
        ${innerHtml}
      </section>
    `;
  }
    ...
    return `
      <div class="section">
        ...
        <div class="actions actions--center">
          <a class="btn btn--gold" href="${waLink(t("whats.home"))}" target="_blank" rel="noopener">${t("home.ctaWhats")}</a>
          <a class="btn btn--ghost" href="#/assinatura">${t("home.ctaPlans")}</a>
        </div>
        </div>
...
function pageServicos() {
  ...
  function pageSingle() {
    ...
    const inicio = `
      <div class="section section--tight">
        ...
        <div class="valueRow">
          <div class="valueCard">
            <div class="valueCard__icon">⚖️</div>
            <div class="valueCard__title">Atuação estratégica</div>
            <div class="valueCard__text">Foco em resultado, ética e segurança jurídica.</div>
          </div>
          <div class="valueCard">
            <div class="valueCard__icon">🧾</div>
            <div class="valueCard__title">Análise individualizada</div>
            <div class="valueCard__text">Cada caso é tratado com cuidado técnico e clareza.</div>
          </div>
          <div class="valueCard">
            <div class="valueCard__icon">🤝</div>
            <div class="valueCard__title">Atendimento humano</div>
            <div class="valueCard__text">Comunicação objetiva e acompanhamento próximo.</div>
          </div>
        </div>
      </div>
    `;
      ...
      <div class="section">
        ...
        <div class="grid grid--2">
          <div class="card">
            <h4>${t("areas.prevTitle")}</h4>
            <p class="notice">${t("areas.prevNotice")}</p>
            <ul class="list">${[1,2,3,4,5,6,7].map(i => `<li>${t("areas.prev."+i)}</li>`).join("")}</ul>
          </div>
          <div class="card">
            <h4>${t("areas.tribTitle")}</h4>
            <ul class="list">${[1,2,3,4,5,6].map(i => `<li>${t("areas.trib."+i)}</li>`).join("")}</ul>
          </div>
        </div>
    ...
    const servicos = `
      <div class="section">
        ...
        <div class="promo promo--split" role="region" aria-label="Direito Previdenciário">
          ...
          <a class="btn btn--gold promo__btn" target="_blank" rel="noopener" href="${waLink(t("whats.services"))}">Falar com a especialista</a>
        </div>
        ...
        <div class="promo promo--split" role="region" aria-label="Atuação Tributária">
    ...
    const agendamento = pageAgendamento().replace('href="#/agendamento"', 'href="#agendamento"');
    const contato = pageContato().replaceAll('#/agendamento', '#agendamento');

    return [
      wrapSection("inicio", t("nav.home"), inicio),
      wrapSection("sobre", t("nav.about"), sobre),
      wrapSection("atuacao", t("nav.areas"), atuacao),
      wrapSection("servicos", t("nav.services"), servicos),
      wrapSection("agendamento", t("nav.schedule"), agendamento),
      wrapSection("contato", t("nav.contact"), contato),
    ].join("\n");
  }
  ...
  function initScrollSpy() {
    const links = Array.from(document.querySelectorAll(".topnav__link")).filter((a) => (a.getAttribute("href") || "").startsWith("#"));
    const map = new Map();
    links.forEach((a) => {
    ...
    // smooth scroll
    links.forEach((a) => {
      if (a.__boundScroll) return;
      a.__boundScroll = true;
  ...
  function render() {
    ...
    // Calendário (na página única)
    if (window.initCalendar) {
      window.initCalendar(currentLang, t);
    }
  }

  // se carregar com hash antigo (SPA), normaliza
